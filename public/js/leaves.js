...
async function getUser() {
  const res = await safeFetch(`${apiBase}/personal-data.php`, { action: "get-user" });
...
function makeExpandableList(listElement, leaves, renderItem) {
  ...
  const render = () => {
    ...
    toShow.forEach(l => listElement.appendChild(renderItem(l)));
...
function renderMyLeaves() {
  ...
  let leaves = filterLeavesByStatus(allLeaves.filter(l => l.user_id === user.id));
  if (!leaves.length) {
...
function renderDepartmentOrAllLeaves() {
  ...
  if (user.role === "manager") {
    ...
    if (!deptLeaves.length) return;

    deptSection?.classList.remove("hidden");
    makeExpandableList(document.getElementById("employee-leaves-list"), deptLeaves, l => renderLeaveActionItem(l));
  }

  if (user.role === "hr" && leavesFiltered.length) {
    allSection?.classList.remove("hidden");
    makeExpandableList(document.getElementById("all-leaves-list"), leavesFiltered, l => renderLeaveActionItem(l));
  }
}
